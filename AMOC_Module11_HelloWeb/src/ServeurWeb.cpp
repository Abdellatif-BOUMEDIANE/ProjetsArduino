...
ServeurWeb::ServeurWeb()
{
  ...
  this->m_webServer->on("/actionneur", HTTPMethod::HTTP_GET,
                        [this]()
                        { this->actionneur(); });
...
void ServeurWeb::tick()
{
  ...
  Serial.println("HandleClient est appelée!");
  TRACE();
}
...
void ServeurWeb::actionneur()
{
  if (!this->etat)
  {
    this->etat = 1;
    this->m_webServer->send(200, "text/json", "{\"id\" : 1,\"etat\": \"allume\"}");
  }
  else
  {
    this->etat = 0;
    this->m_webServer->send(200, "text/json", "{\"id\" : 1,\"etat\": \"eteint\"}");
  }
}