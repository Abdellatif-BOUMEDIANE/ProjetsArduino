    ...
    : m_pin(p_pinBouton), m_actionBoutonPresse(p_actionBouton)
{
    ...
    this->setDerniereDateChangement(0);
...
void Bouton::tick()
{
    ...
    if (dateActuelle - this->getDerniereDateChangement() > this->m_delaiPression)
    {
        ...
        else if (this->getDernierEtatStableBouton() == LOW && this->getEtatBouton() == HIGH)
        {
            // analogWrite(borneSortie, this->getActionBouton().getPourcentageIntensite() * 255 / 100);
